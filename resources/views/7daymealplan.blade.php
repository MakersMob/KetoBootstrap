@extends('layouts.app', ['title' => 'Free 7-Day Keto Meal Plan'])

@section('content')
<section class="welcome food">
	<div class="container">
		<div class="row">
			<div class="col-12">
				<h1 class="linewrap"><span>7-Day Keto Diet Meal Plan</span></h1>
				<h2 class="linewrap"><span>Give the Ketogenic Diet a Test Run with This Free 7-Day Meal Plan</span></h2>
			</div>
		</div>
	</div>
</section>
<section class="content post">
	<div class="container">
		<div class="row">
			<div class="col-12">
				<h2>Day 1</h2>
				<ul class="list-unstyled">
					<li>Breakfast: <a href="https://ketodash.com/recipe/cinnamon-roll-waffle">Cinnamon Roll Waffle</a></li>
					<li>Lunch: <a href="https://ketodash.com/recipe/zucchini-poppers">Zucchini Poppers</a></li>
					<li>Dinner: <a href="https://ketodash.com/recipe/one-pot-shrimp-alfredo">One Pot Shrimp Alfredo</a></li>
					<li>Snack: <a href="https://ketodash.com/recipe/chocolate-chip-cookie-cheesecake-bars">Chocolate Chip Cookie Cheesecake Bars</a></li>
				</ul>
				<h2>Day 2</h2>
				<ul class="list-unstyled">
					<li>Breakfast: <a href="https://ketodash.com/recipe/breakfast-quiche">Breakfast Quiche</a></li>
					<li>Lunch: <a href="https://ketodash.com/recipe/avocado-taco-boats">Avocado Taco Boats</a></li>
					<li>Dinner: <a href="https://ketodash.com/recipe/southwestern-shepherd-pie">Southwest Sheppard's Pie</a></li>
				</ul>
				<h2>Day 3</h2>
				<ul class="list-unstyled">
					<li>Breakfast: <a href="https://ketodash.com/recipe/cream-cheese-scrambled-eggs">Cream Cheese Scrambled Egg (eat both servings)</a></li>
					<li>Lunch: <a href="https://ketodash.com/recipe/zucchini-poppers">Zucchini Poppers</a> (Eat 2)</li>
					<li>Dinner: <a href="https://ketodash.com/recipe/one-pot-shrimp-alfredo">One Pot Shrimp Alfredo</a></li>
					<li>Snack or Dessert: <a href="https://ketodash.com/recipe/chocolate-chip-cookie-cheesecake-bars">Chocolate Chip Cookie Cheesecake Bars</a></li>
				</ul>
				<h2>Day 4</h2>
				<ul class="list-unstyled">
					<li>Breakfast: <a href="https://ketodash.com/recipe/breakfast-quiche">Breakfast Quiche</a></li>
					<li>Lunch: <a href="https://ketodash.com/recipe/avocado-taco-boats">Avocado Taco Boats</a></li>
					<li>Dinner: <a href="https://ketodash.com/recipe/southwestern-shepherd-pie">Southwest Sheppard's Pie</a></li>
				</ul>
				<h2>Day 5</h2>
				<ul class="list-unstyled">
					<li>Breakfast: <a href="https://ketodash.com/recipe/cream-cheese-scrambled-eggs">Cream Cheese Scrambled Egg (eat both servings)</a></li>
					<li>Lunch: <a href="https://ketodash.com/recipe/portobello-pizza">Portobello Pizza</a></li>
					<li>Dinner: <a href="https://ketodash.com/recipe/italian-sausage-pepperjack-meatballs">Italian Sausage &amp; Pepperjack Meatballs</a>, <a href="https://ketodash.com/recipe/loaded-cauliflower-mashed-potatoes">Loaded Cauliflower Mashed Potatoes</a></li>
					<li>Snack or Dessert: <a href="https://ketodash.com/recipe/microwave-brownies">Microwave Brownies</a></li>
				</ul>
				<h2>Day 6</h2>
				<ul class="list-unstyled">
					<li>Breakfast: <a href="https://ketodash.com/recipe/breakfast-quiche">Breakfast Quiche</a></li>
					<li>Lunch: <a href="https://ketodash.com/recipe/zucchini-poppers">Zucchini Poppers</a></li>
					<li>Dinner: <a href="https://ketodash.com/recipe/southwestern-shepherd-pie">Southwest Sheppard's Pie</a></li>
					<li>Snack or Dessert: <a href="https://ketodash.com/recipe/chocolate-chip-cookie-cheesecake-bars">Chocolate Chip Cookie Cheesecake Bars</a></li>
				</ul>
				<h2>Day 7</h2>
				<ul class="list-unstyled">
					<li>Breakfast: <a href="https://ketodash.com/recipe/cream-cheese-scrambled-eggs">Cream Cheese Scrambled Egg (eat both servings)</a></li>
					<li>Lunch: <a href="https://ketodash.com/recipe/portobello-pizza">Portobello Pizza</a></li>
					<li>Dinner: <a href="https://ketodash.com/recipe/italian-sausage-pepperjack-meatballs">Italian Sausage &amp; Pepperjack Meatballs</a>, <a href="https://ketodash.com/recipe/loaded-cauliflower-mashed-potatoes">Loaded Cauliflower Mashed Potatoes</a></li>
					<li>Snack or Dessert: <a href="https://ketodash.com/recipe/microwave-brownies">Microwave Brownies</a></li>
				</ul>
			</div>
		</div>
	</div>
</section>
@endsection